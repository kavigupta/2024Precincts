...
def _load_from_columns(gdf, dem_col, rep_col, oth_col=None, total_col=None, state_name=None):
    """
    ...
    result['STATE_ABBR'] = state_name[:2].upper() if state_name and len(state_name) >= 2 else 'UN'
...
def _load_delaware(gdf):
    ...
    """
    if 'DE24Data_D' not in gdf.columns or 'DE24Data_K' not in gdf.columns or 'DE24Data_G' not in gdf.columns:
        raise ValueError(
...
def _load_oklahoma(gdf):
    ...
    """
    if 'OK24Data_H' not in gdf.columns or 'OK24Data_T' not in gdf.columns or 'OK24Data_G' not in gdf.columns:
        raise ValueError(
...
def _load_vermont(gdf):
    ...
    Republican votes = Total - Dem - Kennedy
    """
    if 'VTData24_D' not in gdf.columns or 'VTData24_K' not in gdf.columns or 'VTData24_T' not in gdf.columns:
        raise ValueError(
...
STATE_COLUMNS = {
    'Alabama': {'dem': 'G24PREDHAR', 'rep': 'G24PRERTRU', 'oth': ['G24PREIKEN', 'G24PREIOLI', 'G24PREISTE', 'G24PREOWRI'], 'total': None},
    ...
    'Connecticut': {'dem': 'PresDem', 'rep': 'PresRep', 'oth': None, 'total': 'PresTot'},
    'District of Columbia': {'dem': 'DC24Data_D', 'rep': 'DC24Data_R', 'oth': None, 'total': 'DC24Data_T'},
    'Florida': {'dem': 'Florida__2', 'rep': 'Florida__1', 'oth': None, 'total': 'Florida__9'},
    'Georgia': {'dem': 'G24PREDHAR', 'rep': 'G24PRERTRU', 'oth': ['G24PREGSTE', 'G24PREICRU', 'G24PREIWES', 'G24PRELOLI'], 'total': None},
    ...
    'Iowa': {'dem': 'votes_dem', 'rep': 'votes_rep', 'oth': None, 'total': 'votes_tota'},
    'Kansas': {'dem': 'DHarris', 'rep': 'RTrump', 'oth': 'Other', 'total': None},
    'Kentucky': {'dem': 'PresDem', 'rep': 'PresRep', 'oth': None, 'total': 'PresTot'},
    'Louisiana': {'dem': 'G24PREDHAR', 'rep': 'G24PRERTRU', 'oth': ['G24PRELOLI', 'G24PREOCRU', 'G24PREOFRU', 'G24PREOKEN', 'G24PREOPRE', 'G24PREOSON', 'G24PREOSTE', 'G24PREOTER', 'G24PREOWES'], 'total': None},
    'Maine': {'dem': 'PresDem', 'rep': 'PresRep', 'oth': None, 'total': 'PresTot'},
    'Maryland': {'dem': 'G24PRESD', 'rep': 'G24PRESR', 'oth': 'G24PRESO', 'total': None},
    'Massachusetts': {'dem': 'PresDem', 'rep': 'PresRep', 'oth': None, 'total': 'PresTot'},
    'Michigan': {'dem': 'PresDem', 'rep': 'PresRep', 'oth': None, 'total': 'PresTot'},  # Data not available per README
    'Minnesota': {'dem': 'MNPrecinct', 'rep': 'MNPrecin_1', 'oth': None, 'total': 'MNPrecin_2'},
    'Mississippi': {'dem': 'PresDem', 'rep': 'PresRep', 'oth': None, 'total': 'PresTot'},
    'Missouri': {'dem': 'HARRIS - D', 'rep': 'TRUMP - RE', 'oth': None, 'total': 'PRESTOT'},
    ...
    'New Hampshire': {'dem': 'PresDem', 'rep': 'PresRep', 'oth': None, 'total': 'PresTot'},
    'New Jersey': {'dem': 'Harris', 'rep': 'Trump', 'oth': 'Other', 'total': None},
    'New Mexico': {'dem': 'NMData24_1', 'rep': 'NMData24_2', 'oth': None, 'total': 'NMData24_3'},
    'New York': {'dem': 'PresDem', 'rep': 'PresRep', 'oth': None, 'total': 'PresTot'},
    'North Carolina': {'dem': 'PRESDEM', 'rep': 'PRESREP', 'oth': None, 'total': 'PRESTOTAL'},
    'North Dakota': {'dem': 'PresDem', 'rep': 'PresRep', 'oth': None, 'total': 'PresTot'},
    'Ohio': {'dem': 'PresDem', 'rep': 'PresRep', 'oth': None, 'total': 'PresTot'},  # Has NaN - will error correctly
    'Oregon': {'dem': 'USP24D', 'rep': 'USP24R', 'oth': None, 'total': 'USP24Tot'},
    'Pennsylvania': {'dem': 'PresDem', 'rep': 'PresRep', 'oth': None, 'total': 'PresTot'},  # Data not available per README
    'Rhode Island': {'dem': 'PresDem', 'rep': 'PresRep', 'oth': None, 'total': 'PresTot'},
    'South Carolina': {'dem': 'Harris', 'rep': 'Trump', 'oth': None, 'total': 'Total'},
    'South Dakota': {'dem': 'G24PRESD', 'rep': 'G24PRESR', 'oth': None, 'total': 'G24PRESTOT'},
    'Tennessee': {'dem': 'votes_dem', 'rep': 'votes_rep', 'oth': None, 'total': 'votes_tota'},
    'Texas': {'dem': 'PresDem', 'rep': 'PresRep', 'oth': None, 'total': 'PresTot'},
    'Utah': {'dem': 'Harris', 'rep': 'Trump', 'oth': None, 'total': 'Total'},
    'Virginia': {'dem': 'G24PRESD', 'rep': 'G24PRESR', 'oth': 'G24PRESO', 'total': None},
    'Washington': {'dem': 'PresDem', 'rep': 'PresRep', 'oth': None, 'total': 'PresTot'},
    'West Virginia': {'dem': 'PresDem', 'rep': 'PresRep', 'oth': None, 'total': 'PresTot'},
    'Wisconsin': {'dem': 'votes_dem', 'rep': 'votes_rep', 'oth': None, 'total': 'votes_tota'},
        ...
        try:
            ...
            if csv_files:
                ...
                if not merged:
                    ...
                    elif state_name == 'Rhode Island' and 'DISTRICTN' in gdf.columns and 'GEOID' in df.columns:
    ...
    for state_dir in state_dirs:
        ...
        if gdf is not None and len(gdf) > 0:
            ...
            if gdf['dem'].sum() > 0 or gdf['rep'].sum() > 0:
                print(f"✓ Success ({len(gdf)} features, dem={gdf['dem'].sum():.0f}, rep={gdf['rep'].sum():.0f})")
